...
void
fe_userlist_insert (session *sess, struct User *newuser, int sel)
{
...
userlist_alpha_cmp (GtkTreeModel *model, GtkTreeIter *iter_a, GtkTreeIter *iter_b, gpointer userdata)
{
	...
	return nick_cmp_alpha (user_a, user_b, (server*)userdata);
}
...
userlist_ops_cmp (GtkTreeModel *model, GtkTreeIter *iter_a, GtkTreeIter *iter_b, gpointer userdata)
{
	...
	return nick_cmp_az_ops ((server*)userdata, user_a, user_b);
}
...
userlist_create_model (session *sess)
{
	...
	gtk_tree_sortable_set_default_sort_func (GTK_TREE_SORTABLE(store), cmp_func, sess->server, NULL);
